...
사용자는 저장된 DB 코스 상세 화면에서 공유 버튼을 누를 수 있다. Web Share API를 지원하는 모바일 브라우저에서는 `navigator.share`를 통해 OS 기본 공유창을 사용하고, 미지원 환경 또는 공유 호출 실패 시에는 Clipboard API로 `/detail/:id` 딥링크를 복사한다.

복사 성공 시 사용자가 동작 결과를 알 수 있도록 안내 메시지를 표시한다. 생성형 ORS 코스는 아직 상세 딥링크로 안정적으로 복원할 수 없으므로 이번 단계에서는 공유 대상에서 제외한다.
