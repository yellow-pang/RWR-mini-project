...
Step 25에서는 ORS 생성형 코스가 같은 길을 일부 공유하거나 가까운 구간을 반복해서 지나갈 때 사용자가 진행 방향을 오해하지 않도록 지도 경로 위에 방향 표시를 제공한다.

결과 카드 축소 지도와 상세 화면 지도는 `geometry.coordinates`가 있는 경우 경로 중간 지점에 방향 화살표를 표시한다. 카카오맵 SDK를 사용할 수 없는 fallback 프리뷰에서도 예시 경로 위에 방향 화살표를 표시한다. 이 변경은 지도 표현 보완이며 API 응답 형식, DB 스키마, 즐겨찾기/이력 정책은 변경하지 않는다.
