...
### 2026.06.01 코스 공유 요구사항 보정 기록

Step 24에서는 코스 상세 화면에 공유 버튼을 추가한다.

사용자는 코스 상세 화면에서 공유 버튼을 누를 수 있다. 저장된 DB 코스는 `/detail/:id` 딥링크를 공유하고, 생성형 ORS 코스는 아직 상세 딥링크로 안정적으로 복원할 수 없으므로 코스 요약 문구와 서비스 URL을 공유한다.

Web Share API를 지원하는 모바일 브라우저에서는 `navigator.share`를 통해 OS 기본 공유창을 사용하고, 미지원 환경 또는 공유 호출 실패 시에는 Clipboard API로 공유 URL을 복사한다. 복사 성공 시 사용자가 동작 결과를 알 수 있도록 안내 메시지를 표시한다.

주소 기반 생성형 코스에서는 기존 저장 코스 즐겨찾기 목적과 달라질 수 있으므로, 이번 단계에서는 상세 화면과 결과 카드에서 즐겨찾기 버튼을 숨기고 공유만 제공한다. 주소 기반 코스 저장/즐겨찾기 정책은 후속 단계에서 별도 기능 변경으로 재정의한다.

### 2026.06.02 지도 경로 방향 표시 요구사항 보정 기록

Step 25에서는 ORS 생성형 코스가 같은 길을 일부 공유하거나 가까운 구간을 반복해서 지나갈 때 사용자가 진행 방향과 이동 순서를 오해하지 않도록 지도 표현을 보완한다.

결과 카드 축소 지도와 상세 화면 지도는 `geometry.coordinates`가 있는 경우 출발/도착 마커, 중간 번호 마커, 원형 방향 마커를 표시한다. 전체 경로는 연한 초록색으로, 출발 구간은 진한 초록색으로 구분한다.

카카오맵 SDK를 사용할 수 없는 fallback 프리뷰에서도 예시 경로 위에 번호/방향/출발/도착 마커를 표시한다. 이 변경은 지도 표현 보완이며 API 응답 형식, DB 스키마, 즐겨찾기/이력 정책은 변경하지 않는다. GPS 현재 위치 기반 현재 구간 강조, 하단 다음 이동 카드, 겹침 구간 offset 처리는 후속 단계에서 별도 요구사항으로 다룬다.

### 2026.06.03 출발-도착 코스 생성 요구사항 보정 기록

Step 26에서는 주소 기반 생성형 코스에 `출발-도착 코스` 모드를 추가한다.

사용자는 홈 화면에서 `순환 코스` 또는 `출발-도착 코스`를 선택할 수 있다. 순환 코스는 기존처럼 출발지 하나로 생성하고, 출발-도착 코스는 출발지와 도착지를 모두 좌표로 확정한 뒤에만 코스 생성 버튼을 활성화한다.

출발-도착 코스의 거리 조건은 출발지부터 도착지까지의 전체 예상 거리로 처리한다. 출발지와 도착지 사이의 기본 보행 거리가 선택 거리보다 길 것으로 판단되면 서버는 랜덤 경유지를 최소화하고, 응답 `meta.notice`에 안내 문구를 포함한다.

현재 위치 사용 버튼은 출발지 입력 보조로만 제공한다. 도착지는 주소 검색으로 선택한다. ORS 경로 생성에 실패하면 저장 DB 코스를 fallback으로 반환하지 않는다. 대신 서버가 랜덤 경유지를 바꿔 최대 3회까지 재시도하고, 최종 실패 시 사용자가 출발지/도착지를 조정하거나 다시 생성할 수 있도록 실패 메시지를 반환한다.

생성형 출발-도착 코스는 기존 생성형 ORS 코스와 동일하게 상세 화면과 공유 기능을 제공하지만, DB 저장 정책이 아직 없으므로 즐겨찾기 버튼은 숨긴다.

### 2026.06.03 추천 목표 정확도 요구사항 보정 기록

Step 27에서는 거리, 시간, 운동 유형 조건을 동시에 고르는 구조를 `거리 기준`과 `시간 기준`으로 분리한다.

사용자는 홈 화면에서 코스 방식, 추천 기준, 운동 유형을 차례로 선택한다. 거리 기준을 선택하면 `1km / 2km / 3km / 5km / 직접 설정` 중 목표 거리를 고르고, 운동 유형별 속도로 예상 시간을 확인한다. 시간 기준을 선택하면 `15분 / 30분 / 45분 / 60분 / 직접 설정` 중 목표 시간을 고르고, 운동 유형별 속도로 예상 거리를 확인한다.

운동 유형별 기본 속도는 걷기 4km/h, 조깅 7km/h, 러닝 9km/h로 계산한다. 시간 기준 추천은 내부적으로 목표 거리를 계산하고, 거리 기준 추천은 예상 시간을 계산한다.

서버는 ORS가 반환한 실제 `summary.distance`, `summary.duration`을 기준으로 여러 랜덤 후보를 검증한다. 허용 범위 안에 들어오는 후보가 여러 개면 목표 거리와 가장 가까운 후보를 반환하고, 조건에 맞는 후보가 없으면 저장 DB 코스를 억지로 fallback하지 않고 실패 안내를 반환한다.

결과 화면은 `정확한 거리/시간`이 아니라 `예상 거리`, `예상 시간`, `실제 추천 거리`, `목표와의 차이`를 표시한다.

### 2026.06.03 지도 중심 홈 화면과 조건 조작 요구사항 보정 기록

Step 28에서는 홈 화면을 조건 입력 중심 화면에서 지도 중심 화면으로 보정한다.

모바일에서는 지도 영역을 화면 대부분에 배치하고, 코스 조건은 화면 아래에서 올라온 패널처럼 보이는 하단 고정 패널에 배치한다. 1차 구현에서는 드래그 가능한 3단계 바텀시트가 아니라 고정 하단 패널과 내부 스크롤로 처리한다. 데스크톱에서는 왼쪽 조건 패널과 오른쪽 지도 레이아웃을 사용한다.

거리 기준 추천의 거리 프리셋은 운동 유형별로 다르게 보여준다. 걷기는 `1km / 2km / 3km / 직접 설정`, 조깅은 `2km / 3km / 5km / 직접 설정`, 러닝은 `3km / 5km / 7km / 직접 설정`을 사용한다. 시간 기준 추천은 기존 `15분 / 30분 / 45분 / 60분 / 직접 설정` 프리셋을 유지한다.

출발-도착 코스에서는 출발지와 도착지를 서로 바꾸는 버튼을 제공한다. 또한 `가볍게 / 적당히 / 많이` 우회 강도를 선택할 수 있다. 우회 강도는 출발-도착 코스의 랜덤 경유지 개수와 경유지 offset을 조정하는 데 사용하되, Step 27의 실제 거리 목표 검증을 우선한다.

POI 기반 경유지 선택은 외부 API와 fallback 정책이 필요하므로 이번 Step에서는 구현하지 않고 후속 Step에서 별도로 검토한다.

### 2026.06.03 Step 28 UX 재보정 기록

Step 28 구현 확인 후, RWR의 첫 화면은 지도 앱처럼 지도를 크게 탐색하는 화면보다 코스 추천 조건을 빠르게 입력하는 화면이 더 적합하다고 판단했다. 따라서 홈 화면은 조건 입력 중심 구조로 되돌리고, 주소가 선택된 뒤에만 작은 위치 확인 지도를 표시한다.

추천 결과 화면은 사용자가 실제 코스를 확인하는 단계이므로 지도 중심 구조로 보정한다. 결과 화면에서는 큰 지도에 추천 경로를 먼저 보여주고, 아래에서 목표 거리, 실제 추천 거리, 예상 시간, 목표와의 차이, 다시 추천 동작을 확인한다.

정리하면 Step 28의 최종 UX 기준은 `입력 전에는 조건 중심`, `주소 선택 후에는 작은 위치 확인 지도`, `추천 후에는 지도 중심 결과 화면`이다.

### 2026.06.03 POI 기반 경유지 후보 요구사항 보정 기록

Step 29에서는 생성형 코스의 랜덤성을 사용자가 더 납득할 수 있도록 POI 기반 경유지 후보를 추가한다.

사용자는 홈 화면에서 `코스 분위기`를 선택할 수 있다. 초기 선택지는 `아무거나`, `공원 위주`, `하천/산책로`, `카페 근처`, `편의점 근처`로 제한한다. 이 선택은 특정 장소를 반드시 방문하라는 목적지가 아니라, 코스가 어떤 분위기의 장소 근처를 지나면 좋을지 알려주는 힌트로 사용한다.

서버는 Kakao Local API로 출발지, 도착지, 중간 지점 또는 출발지 주변 POI 후보를 검색한다. 출발-도착 코스와 순환 코스 모두 POI 후보를 먼저 경유지 후보로 사용하고, POI 후보가 없거나 API 호출이 실패하거나 ORS 목표 거리 검증을 통과하지 못하면 기존 랜덤 경유지 또는 ORS round trip 방식으로 fallback한다.

POI 후보가 있더라도 최종 추천 여부는 기존 Step 27의 ORS 실제 거리/시간 검증을 우선한다. 결과 화면에는 POI를 목적지로 강제하지 않는 표현으로 코스 분위기 추천 이유를 표시한다.
...
## Step 30 보정 기록: POI 선호 토글과 도심지 매칭 보정

작성일: 2026.06.03

- `코스 분위기`는 단일 선택 목록에서 선택적 토글 방식으로 보정한다.
- 기본 상태는 `추천 균형 코스`이며, 사용자는 필요할 때 `공원/녹지`, `카페 근처`, `편의점 근처`를 추가로 선택한다.
- POI 선호 토글은 MVP에서 최대 2개까지만 허용한다.
- 카페와 편의점처럼 Kakao Local 카테고리 코드가 안정적인 항목은 카테고리 검색을 우선 사용한다.
- 카테고리 검색 결과가 없거나 공원/녹지처럼 키워드가 더 자연스러운 항목은 키워드 검색을 fallback 또는 기본 검색으로 사용한다.
- POI 후보가 부족하면 기존 랜덤 경유지 또는 ORS round trip fallback을 유지한다.
