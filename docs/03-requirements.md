...
| 즐겨찾기 저장/해제  | localStorage 코스 ID 저장·삭제, 아이콘 상태 반영  |  **P1**  |   🟢 낮음   |       1h       |
| 즐겨찾기 목록 화면  | 저장된 코스 목록 + 빈 상태 UI                     |  **P1**  |   🟢 낮음   |       1h       |
| 최근 추천 이력 저장 | 최대 10건 localStorage 저장 로직                  |  **P1**  |   🟢 낮음   |       1h       |
...
| GPS 위치 기반 필터  | Geolocation API, Haversine 거리 계산              |  **P2**  |   🟡 중간   |     1~1.5h     |
...
| ID    | 요구사항                                                  | 우선순위 | 검증 방법                                   |
| ----- | --------------------------------------------------------- | :------: | ------------------------------------------- |
| FR-15 | 사용자는 코스를 즐겨찾기에 저장할 수 있다                 |    P1    | 저장 버튼 클릭 → localStorage 키 확인       |
| FR-16 | 즐겨찾기 아이콘이 저장 상태에 따라 시각적으로 구분된다    |    P1    | 저장/미저장 상태 아이콘 색상·모양 확인      |
| FR-17 | 즐겨찾기는 새로고침 후에도 유지된다                       |    P1    | 저장 후 새로고침 → localStorage 데이터 확인 |
| FR-18 | 즐겨찾기 화면에서 저장된 코스 목록을 확인할 수 있다       |    P1    | 즐겨찾기 탭 이동 → 목록 렌더링 확인         |
| FR-19 | 즐겨찾기 해제 시 목록에서 즉시 제거된다                   |    P1    | 해제 버튼 클릭 → 목록 즉시 업데이트 확인    |
| FR-20 | 즐겨찾기가 없을 때 빈 상태(empty state) 메시지가 표시된다 |    P1    | localStorage 비운 뒤 즐겨찾기 탭 확인       |

### 3.5 최근 추천 이력

| ID    | 요구사항                                                               | 우선순위 | 검증 방법                           |
| ----- | ---------------------------------------------------------------------- | :------: | ----------------------------------- |
| FR-21 | 코스 추천 시 해당 코스가 최근 이력에 자동 저장된다                     |    P1    | 추천 후 localStorage 이력 키 확인   |
...
| FR-24 | 이력이 없을 때 빈 상태 메시지가 표시된다                               |    P1    | localStorage 비운 뒤 이력 탭 확인   |
...
| ID     | 요구사항                                                                            | 기준                   |
| ------ | ----------------------------------------------------------------------------------- | ---------------------- |
| NFR-07 | 즐겨찾기 데이터는 localStorage에 저장되어 브라우저 세션 종료 후에도 유지되어야 한다 | localStorage 영구 저장 |
| NFR-08 | 최근 추천 이력은 localStorage에 저장되어야 한다                                     | localStorage 저장 확인 |
| NFR-09 | localStorage 저장 실패 시 서비스가 중단되지 않아야 한다                             | try-catch 처리         |
