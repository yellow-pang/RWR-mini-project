...
### [일 10:00~12:00] 📍 GPS 기반 자동 코스 추천 옵션 (도전 기능)

```
□ Geolocation API — navigator.geolocation.getCurrentPosition
□ 추천 방식을 기존 랜덤 추천 / GPS 자동 추천으로 분리
□ Step 18에서는 위치 취득 후 기존 랜덤 추천으로 fallback
□ 실제 도로망 기반 경로 생성은 Step 19 이후 ORS 연동으로 확장
□ 위치 권한 거부 시 기존 랜덤 추천으로 fallback
□ 커밋: "feat: GPS 기반 추천 옵션 추가"
```

**완료 기준**: GPS 추천 옵션 선택 시 위치 권한 요청이 발생하고, 위치 허용/거부 모두 기존 랜덤 추천 fallback과 안내 문구로 동작

> 2026.05.31 보정: 초기 일정의 "Haversine 기반 가까운 코스 정렬"은 MVP 고정 코스를 빠르게 활용하기 위한 계획이었다. 최종 목표는 현재 위치 기반 랜덤 운동 코스 자동 추천이므로, 가까운 코스 정렬은 메인 방향에서 제외하고 fallback/보조 기능 후보로 남긴다.
