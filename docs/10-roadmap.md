...
| **GPS 기반 추천 옵션** (v1.0 도전) | 현재 위치 탐지 후 자동 코스 추천 기반과 fallback 제공 | Geolocation API           | 1일       |
...
### 2026.05.31 GPS 로드맵 보정 기록

초기 로드맵의 "GPS 위치 기반 필터"는 저장된 코스 중 근처 코스를 우선 추천하는 MVP 확장안이었다. Step 18에서 제품 방향을 다시 확인하면서 GPS 기능은 현재 위치, 목표 거리, 소요 시간, 운동 유형을 바탕으로 랜덤 운동 코스를 생성/추천하는 방향으로 보정했다.

v1.0에서는 GPS 추천 옵션과 기존 랜덤 추천 fallback 기반을 먼저 제공하고, 실제 도로망 기반 경로 생성과 지도 polyline 표시는 ORS Round Trip 연동 단계에서 확장한다.
