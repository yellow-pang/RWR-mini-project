...
### 2026.06.01 코스 공유 기능 보정 기록

Step 24에서는 코스 상세 화면에 공유 기능을 추가한다. 사용자는 코스 상세 화면에서 공유 버튼을 눌러 모바일 브라우저의 기본 공유창을 열거나, Web Share API를 지원하지 않는 환경에서는 공유 URL을 클립보드에 복사할 수 있다.

저장된 DB 코스의 공유 URL은 `/detail/:id` 형식의 딥링크를 사용한다. 기존 내부 상세 경로인 `/courses/:id`는 호환을 위해 유지한다. 생성형 ORS 코스는 아직 영속 저장 정책과 복원 가능한 ID가 없으므로 코스 요약 문구와 서비스 URL을 공유하고, 즐겨찾기 버튼은 노출하지 않는다.

### 2026.06.02 지도 경로 방향 표시 보정 기록

Step 25에서는 주소 기반 ORS 생성형 코스가 지도 위에서 겹쳐 보일 때 진행 방향과 이동 순서를 더 쉽게 이해할 수 있도록 지도 표현을 보완한다.

카카오맵 SDK가 정상 로드된 경우 실제 경로 좌표를 기준으로 출발/도착 마커, 중간 번호 마커, 원형 방향 마커를 표시한다. 전체 경로는 연한 초록색으로, 출발 구간은 진한 초록색으로 구분해 사용자가 처음 가야 할 방향을 더 쉽게 파악할 수 있게 한다.

SDK가 없거나 좌표가 없어 SVG 프리뷰로 fallback되는 경우에도 예시 경로 위에 번호/방향/출발/도착 마커를 표시해 지도 표현의 의도를 유지한다. GPS 현재 위치 기반 현재 구간 강조, 하단 다음 이동 카드, 겹침 구간 offset 처리는 후속 단계에서 별도로 검토한다.

### 2026.06.03 출발-도착 코스 생성 보정 기록

Step 26에서는 기존 주소 기반 순환 코스에 더해 출발지와 목적지를 따로 두는 산책형 이동 코스 생성을 추가한다.

사용자는 홈 화면에서 `순환 코스`와 `출발-도착 코스` 중 하나를 선택한다. 순환 코스는 기존처럼 출발 주소 또는 현재 위치를 기준으로 ORS Round Trip 경로를 생성한다. 출발-도착 코스는 출발지와 도착지를 각각 선택하고, 서버가 두 지점 사이에 랜덤 경유지를 더해 ORS Directions API로 보행 경로를 생성한다.

출발-도착 코스에서 사용자가 선택한 거리 조건은 출발지부터 도착지까지의 전체 예상 거리로 해석한다. 출발지와 도착지 사이의 기본 거리가 선택 거리보다 긴 경우에는 우회 경유지를 줄이고 안내 문구를 표시한다.

현재 위치 사용은 출발지 입력 보조로만 제공한다. 도착지는 주소 검색으로 선택한다. 생성형 출발-도착 코스도 아직 영속 저장 정책이 없으므로 즐겨찾기는 숨기고 공유만 제공한다. ORS 경로 생성이 실패하면 저장 DB 코스 fallback은 하지 않고, 서버 내부에서 랜덤 경유지를 바꿔 최대 3회까지 재시도한 뒤 최종 실패 시 주소 조정 또는 다시 생성을 안내한다.
