...
Step 24에서는 코스 상세 화면에 공유 기능을 추가한다. 사용자는 저장된 DB 코스 상세 화면에서 공유 버튼을 눌러 모바일 브라우저의 기본 공유창을 열거나, Web Share API를 지원하지 않는 환경에서는 상세 딥링크를 클립보드에 복사할 수 있다.

공유 URL은 `/detail/:id` 형식의 딥링크를 사용한다. 기존 내부 상세 경로인 `/courses/:id`는 호환을 위해 유지한다. 생성형 ORS 코스는 아직 영속 저장 정책과 복원 가능한 ID가 없으므로 이번 단계에서는 공유 대상에서 제외한다.
