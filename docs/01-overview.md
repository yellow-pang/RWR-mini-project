...
Step 25에서는 주소 기반 ORS 생성형 코스가 지도 위에서 겹쳐 보일 때 진행 방향을 더 쉽게 이해할 수 있도록 경로 위 방향 화살표를 추가한다.

카카오맵 SDK가 정상 로드된 경우 실제 경로 좌표를 기준으로 중간 지점에 방향 오버레이를 표시한다. SDK가 없거나 좌표가 없어 SVG 프리뷰로 fallback되는 경우에도 예시 경로 위에 방향 화살표를 표시해 사용자가 코스 진행 방향을 인지할 수 있게 한다.
