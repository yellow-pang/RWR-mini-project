...
### 2026.05.31 GPS 생성형 코스 데이터 보정 기록

이 문서는 MVP 구현을 위해 PostgreSQL의 `courses` 테이블에 저장된 고정 코스, 즐겨찾기, 최근 추천 이력을 기준으로 작성되었다.
...
erDiagram
    COURSES {
        ...
        varchar type "운동 유형 (walk/jogging/running)"
        varchar mood "분위기 태그 (park/river/city/forest)"
...
| `type`        | `VARCHAR(20)`  | `'walk'`, `'jogging'`, `'running'`        | `'jogging'`           |
| `mood`        | `VARCHAR(20)`  | `'park'`, `'river'`, `'city'`, `'forest'` | `'park'`              |
...
CREATE TABLE IF NOT EXISTS courses (
  ...
  type         VARCHAR(20)  NOT NULL CHECK (type IN ('walk', 'jogging', 'running')),
  mood         VARCHAR(20)  NOT NULL CHECK (mood IN ('park', 'river', 'city', 'forest')),
...
| `type`     | `string`  | 선택 | `walk`, `jogging`, `running` | 유형 필터     |

```
GET /api/courses?distance=3&time=30&type=jogging
```
...
```
GET /api/courses/random?distance=3&time=30&type=jogging&exclude=route-001
```
