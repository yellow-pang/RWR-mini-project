...
| **버전**      | v1.0 (MVP)                                                                   |
| **유형**      | 반응형 웹 서비스 (SPA + REST API)                                            |
| **플랫폼**    | 모바일 브라우저 우선, PC 브라우저 지원                                       |
| **기술 스택** | React 18 + Vite 5 + Express 4 + Node.js 20 + PostgreSQL 16 + JavaScript ES6+ |
| **데이터**    | PostgreSQL DB (코스 데이터 + 즐겨찾기 + 이력)                                |
| **저장소**    | PostgreSQL (서버) + localStorage (익명 UUID만)                               |
| **배포**      | Docker Compose + Nginx + Ubuntu VM (자체 서버) + GitHub Actions CI/CD        |
| **개발 기간** | 3일 (2026.05.29 금 ~ 05.31 일, 실작업 약 30시간)                             |
