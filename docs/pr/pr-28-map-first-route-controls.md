# PR #28. 결과 지도 중심 화면과 코스 조건 조작 개선
...
```text
[Step 28] 결과 지도 중심 화면과 코스 조건 조작 개선
```
...
홈 화면은 조건 입력 중심 구조로 유지하고, 주소 선택 후에만 작은 위치 확인 지도를 보여주도록 보정했다. 추천 결과 화면은 큰 지도 중심 구조로 변경해 사용자가 생성된 코스를 먼저 확인할 수 있게 했다.
...
| 수정 | `client/src/pages/HomePage.jsx` | 조건 중심 구조 유지, 작은 위치 확인 지도, 출발/도착 바꾸기, 우회 강도 UI 추가 |
| 수정 | `client/src/pages/ResultPage.jsx` | 추천 결과 큰 지도 영역 추가, 중복 작은 지도 제거, 다시 추천 요청에 `detourLevel` 유지 |
| 수정 | `client/src/api/routes.js` | point-to-point 요청 body에 `detourLevel` 포함 |
| 수정 | `client/src/index.css` | 홈 위치 확인 지도와 결과 큰 지도 레이아웃 추가 |
...
- 주소 선택 후 홈 화면에 작은 위치 확인 지도가 표시된다.
- 결과 화면은 큰 지도에 추천 경로를 먼저 보여준다.
