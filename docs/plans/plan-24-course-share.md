...
| 생성형 ORS 코스 공유 | 이번 Step에서는 제한 또는 fallback 문구 처리 | 생성형 코스는 DB에 영속 ID가 없을 수 있어 딥링크 복원이 불안정함           |
...
- 생성형 ORS 코스처럼 안정적인 상세 딥링크가 없는 경우 공유 버튼을 숨기거나 안내 메시지로 처리한다.
...
권장: 이번 Step에서는 DB 코스만 공유 대상으로 삼고, 생성형 ORS 코스는 공유 버튼을 숨긴다.
...
| 생성형 ORS 코스 영속 공유           | 생성형 코스 저장 정책과 ID 설계가 필요함                |
...
| 생성형 코스 처리    | ORS 생성 코스에서 공유 버튼 숨김 또는 안내 처리 확인               |
