...
> **목적**: 랜덤 생성 코스가 지도에서 겹쳐 보일 때 사용자가 진행 방향을 더 쉽게 이해하도록 경로 위 방향 표시를 추가한다.  
...
- `MapView`에서 `routeCoordinates`가 있는 경우 경로 위에 진행 방향 화살표를 표시한다.
- 결과 카드의 compact 지도와 상세 화면 지도 모두 방향 힌트를 제공한다.
- 카카오맵 SDK를 사용할 수 없는 fallback `MapPreview`에도 방향 힌트를 추가한다.
...
| 항목 | 결정 | 이유 |
| ---- | ---- | ---- |
| 방향 표시 방식 | 경로 중간 지점의 화살표 오버레이 | 겹치는 선에서도 진행 방향을 직접 보여줄 수 있음 |
...
| fallback | SVG 경로 위 화살표 | SDK 미로딩 상태에서도 같은 사용성을 유지 |
...
| 파일 | 변경 내용 |
| ---- | --------- |
| `client/src/components/MapView.jsx` | 경로 방향 계산, 카카오맵 방향 오버레이 추가 |
| `client/src/components/MapView.css` | 방향 화살표 오버레이 스타일 추가 |
| `client/src/components/MapPreview.jsx` | fallback SVG 방향 화살표 추가 |
| `client/src/components/MapPreview.css` | fallback 방향 화살표 스타일 추가 |
...
- ORS 생성형 코스 결과 카드 지도에 방향 화살표가 표시되는지 확인
- 상세 화면 지도에 방향 화살표가 표시되는지 확인
- SDK fallback `MapPreview`에도 방향 화살표가 표시되는지 확인
